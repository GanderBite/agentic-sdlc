...
const priors = existsSync(priorsPath)
  ? JSON.parse(readFileSync(priorsPath, 'utf8'))
  : empty;
...
const mergeMultiplierBucket = (existing = {}, patchEntries = {}) => {
  ...
  for (const [name, delta] of Object.entries(patchEntries)) {
    ...
    const newSumSq = (cur._sumSq ?? Math.pow(cur.stddev, 2) * cur.n + cur.mean_ratio * cur.mean_ratio * cur.n) + dsq;
    const newN = cur.n + dn;
    if (newN === 0) { out[name] = { mean_ratio: 1.0, n: 0, stddev: 0 }; continue; }
...
const existingHints = new Map(
  (priors.wave_invariant_hints ?? []).map((h) => [h.pattern, h]),
);
for (const add of patch.wave_invariant_hints_add ?? []) {
  if (!add?.pattern) continue;
  const cur = existingHints.get(add.pattern);
  if (cur) {
    cur.evidence_sprints = [...new Set([...(cur.evidence_sprints ?? []), ...(add.evidence_sprints ?? [])])];
...
const existingModes = new Map(
  (priors.verification_failure_modes ?? []).map((m) => [m.command, m]),
);
