...
export const ArchitectureSchema = z.object({
  ...
  system_sections_added: z
    .array(z.string())
    .describe('Headings added to docs/ARCHITECTURE.md in this run (e.g. "Modules/<new-module>"). Empty when no system-level change.'),
  system_sections_modified: z
    .array(z.string())
    .describe('Headings modified in place in docs/ARCHITECTURE.md. Empty when no system-level change.'),
  system_diff_summary: z
    .string()
    .min(1)
    .describe('One-or-two-sentence summary of system-level changes (or "no system-level architectural change required for this feature"). Quoted by the human approve-arch gate.'),

  // --- Feature-level (.planning/features/ARCHITECTURE-<slug>.md) ---
  feature_architecture_decision: z
    .enum(['inherit', 'derive'])
    .describe('"inherit" = feature follows the system architecture as-is (no per-feature file written). "derive" = feature has its own internal style; the .planning/features/ARCHITECTURE-<slug>.md file IS written.'),
  feature_architecture_path: z
    .string()
    .nullable()
    .describe('Path to the per-feature architecture file when feature_architecture_decision is "derive"; null otherwise. Format: ".planning/features/ARCHITECTURE-<slug>.md".'),
  feature_style: z
    .string()
    .nullable()
    .describe('The chosen feature-level style when deriving (e.g. "hexagonal", "layered", "transactional-script", "vertical-slice", "event-sourced", "ports-and-adapters", "clean"). Null when inheriting.'),

  // --- Enriched feature spec (always written) ---
  enriched_path: z
    .string()
    .describe('Path to the enriched feature spec, normally `.planning/features/<slug>.enriched.md`. Globbed by downstream compose-plan body steps as the source-of-truth for task composition.'),
});
