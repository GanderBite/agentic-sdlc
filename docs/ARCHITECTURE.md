...
|---|---|---|---|
| `auth` | `user_credential`, `refresh_token`, `csrf_token` (if stored) | `auth.login`, `auth.logout`, `auth.refresh`, `auth.me` | Brief §7: JWT + refresh rotation + CSRF + argon2. Single concern, single module. |
| `accounts` | `user`, `patient_profile`, `doctor_profile`, `specialization`, `doctor_specialization` | `patients.*`, `doctors.*`, `specializations.list` | All identity + profile + the seeded reference list. Doctor↔specialization M2M lives here because it is profile data. |
