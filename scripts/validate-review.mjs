...
if (argv.length === 0) {
  process.stderr.write('usage: validate-review.mjs <review-{wave}.json> [<findings-{wave}.json>]\n');
  process.exit(1);
}
...
const readJson = (path) => {
  ...
  catch (e) { emit('blocking', 'parse_error', `cannot parse ${path}: ${e.message}`, { path }); return null; }
};
...
// --- review-{wave}.json ----------------------------------------------------
if (!review) {
  emit('blocking', 'review_missing', `cannot read review file: ${reviewPath}`, { path: reviewPath });
} else {
  ...
  if (!['pass', 'blocked', 'failed', 'partial', 'reviewer_overload'].includes(review.verdict)) {
    emit('blocking', 'review_verdict_invalid', `review.verdict invalid: ${JSON.stringify(review.verdict)}`);
  }
  if (!Array.isArray(review.tasks)) {
    ...
    for (const t of review.tasks) {
      ...
      if (!['pass', 'fail', 'partial'].includes(t.verdict)) {
        emit('blocking', 'review_task_verdict_invalid', `task ${t.task_id} verdict invalid: ${JSON.stringify(t.verdict)}`, { task_id: t.task_id });
      }
      if (!Array.isArray(t.gates)) {
        emit('blocking', 'review_task_gates_invalid', `task ${t.task_id} gates must be an array`, { task_id: t.task_id });
        continue;
      }
      for (const g of t.gates) {
        if (!g || typeof g !== 'object') continue;
        if (!['tests', 'lint', 'build', 'files_exist', 'custom'].includes(g.kind)) {
          emit('high', 'review_gate_kind_invalid', `task ${t.task_id} gate has invalid kind: ${g.kind}`, { task_id: t.task_id, kind: g.kind });
        }
        if (typeof g.cmd !== 'string') {
          emit('high', 'review_gate_cmd_missing', `task ${t.task_id} gate missing cmd`, { task_id: t.task_id });
        }
        if (typeof g.exit !== 'number') {
          emit('high', 'review_gate_exit_invalid', `task ${t.task_id} gate exit must be number`, { task_id: t.task_id });
        }
      }
    }
  }
...
// --- findings-{wave}.json --------------------------------------------------
if (!findings) {
  emit('blocking', 'findings_missing', `cannot read findings file: ${findingsPath}`, { path: findingsPath });
} else {
  if (!Array.isArray(findings.findings)) {
    ...
    for (const f of findings.findings) {
      ...
      if (typeof f.id !== 'string' || !/^F-\d+$/.test(f.id)) {
        emit('high', 'finding_id_format', `finding id should match /^F-\\d+$/, got ${JSON.stringify(f.id)}`);
      ...
      if (!['blocking', 'high', 'medium', 'low', 'info'].includes(f.severity)) {
        emit('blocking', 'finding_severity_invalid', `finding ${f.id ?? '?'} severity invalid: ${JSON.stringify(f.severity)}`, { id: f.id });
      }
      if (!['security', 'architecture', 'performance', 'duplication', 'style'].includes(f.category)) {
        emit('high', 'finding_category_invalid', `finding ${f.id ?? '?'} category invalid: ${JSON.stringify(f.category)}`, { id: f.id });
      }
      if (typeof f.summary !== 'string' || !f.summary) {
        emit('high', 'finding_summary_missing', `finding ${f.id ?? '?'} summary missing`, { id: f.id });
      }
      ...
      if (typeof f.file === 'string' && f.file) {
        ...
        if (!existsSync(abs)) {
          emit('blocking', 'finding_file_missing', `finding ${f.id ?? '?'} references missing file: ${f.file}`, { id: f.id, file: f.file });
          ...
          if (lineCount > 0 && f.line > lineCount) {
            emit('high', 'finding_line_out_of_range', `finding ${f.id ?? '?'} line ${f.line} > file length ${lineCount}`, { id: f.id, file: f.file, line: f.line, file_lines: lineCount });
          }
        }
      }
    }
    if (blocking > 5) {
      emit('blocking', 'findings_blocking_overload', `>${5} blocking findings (${blocking}) — reviewer panicking; fail the wave`, { blocking });
    }
  }
}
...
if (fatal > 0) {
  process.stderr.write(`validate-review: ${fatal} blocking error(s); ${errors.length - fatal} other\n`);
  process.exit(1);
}
process.exit(0);
